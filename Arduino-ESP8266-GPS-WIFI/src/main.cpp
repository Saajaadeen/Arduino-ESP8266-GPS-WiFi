...
void setup() {
    ...
    Serial.begin(9600);               // CHANGE TO YOUR SCREEN SERIAL
    gpsSerial.begin(9600);            // BN220 SERIAL
    // Startup Delay of 5 seconds
    delay(5000);
    Serial.println();
    
    ledStartup(5);                    // FUNCTION CALL
    createWebServer();                // FUNCTION CALL
}
void loop() {
    displayGPSData();                 // FUNCTION CALL
}
void createWebServer() {
...
void ledStartup(int times) {
    ...
    for (int i = 0; i < times; i++) {        // ON BOARD LED STARTUP BLINK
